...
function Part({ status, children }: { status: string; children: ReactNode }) {
  switch (status) {
    case "date":
      return (
        <p className="group-hover:brightness-200 min-w-32 pt-4 pl-2 font-sans font-semibold text-slate-500">
          {children}
        </p>
      );
    case "title":
      return (
        <p className="group-hover:text-lime-700 font-sans font-semibold text-slate-100">
          {children}
        </p>
      );
    case "subtitle":
